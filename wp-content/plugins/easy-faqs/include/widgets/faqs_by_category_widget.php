...
class FAQsByCategoryWidget extends WP_Widget
{
	function __construct(){
		$widget_ops = array('classname' => 'FAQsByCategoryWidget', 'description' => 'Displays a list of FAQs grouped by Category.' );
		parent::__construct('FAQsByCategoryWidget', 'Easy FAQs Grouped By Category', $widget_ops);
	}
	
	function FAQsByCategoryWidget(){		
		$this->__construct();
	}
	...
	function form($instance){
		...
		$categories_accordion_style = $instance['categories_accordion_style'];
		...
		<div class="gp_widget_form_wrapper">
			...
			<p>
				...
				<?php EasyFAQs_Config::output_theme_selector($this->get_field_id('theme'), $this->get_field_name('theme'), $theme); ?>
			...
			<fieldset class="radio_text_input">
				...
				<p><input class="widefat" id="<?php echo $this->get_field_id('quick_links'); ?>" name="<?php echo $this->get_field_name('quick_links'); ?>" type="checkbox" value="1" <?php if($quick_links){ ?>checked="CHECKED"<?php } ?> data-shortcode-key="quicklinks" /><label for="<?php echo $this->get_field_id('quick_links'); ?>">Quick Links</label></p>
			...
			<fieldset class="radio_text_input">
				<legend>FAQs Accordion Style:</legend>
			...
			<fieldset class="radio_text_input">
				<legend>Categories Accordion Style:</legend>
				<label title="Normal Style">
					<input type="radio" value="normal" id="sc_gen_style_normal" name="<?php echo $this->get_field_name('categories_accordion_style'); ?>" <?php if($categories_accordion_style == "normal"): ?>checked="CHECKED"<?php endif; ?> data-shortcode-key="categories_accordion" >
					<span>Normal Style</span>
				</label><br/>
				<label title="Accordion Style - First FAQ Visible">
					<input type="radio" value="accordion" id="sc_gen_style_accordion_first_open" name="<?php echo $this->get_field_name('categories_accordion_style'); ?>" <?php if(!$easy_faqs->is_pro): ?>disabled="disabled"<?php endif; ?> <?php if($categories_accordion_style == "accordion"): ?>checked="CHECKED"<?php endif; ?> data-shortcode-key="categories_accordion" >
					<span>Accordion Style - First Category Visible <?php if(!$easy_faqs->is_pro): ?><br/><strong>Easy FAQs Pro Required.</strong> <a href="https://goldplugins.com/our-plugins/easy-faqs-details/upgrade-to-easy-faqs-pro/?utm_source=list_faqs_widget&utm_campaign=up
					grade" target="_blank"><?php echo $easy_faqs->get_str('FAQ_UPGRADE_TEXT'); ?></a><?php endif; ?> </span>
				</label><br/>
				<label title="Accordion Style - All Categories Start Collapsed">
					<input type="radio" value="accordion-collapsed" id="sc_gen_style_accordion_closed" name="<?php echo $this->get_field_name('categories_accordion_style'); ?>" <?php if(!$easy_faqs->is_pro): ?>disabled="disabled"<?php endif; ?> <?php if($categories_accordion_style == "accordion-collapsed"): ?>checked="CHECKED"<?php endif; ?> data-shortcode-key="categories_accordion" >
					<span>Accordion Style - All Categories Start Collapsed <?php if(!$easy_faqs->is_pro): ?><br/><strong>Easy FAQs Pro Required.</strong> <a href="https://goldplugins.com/our-plugins/easy-faqs-details/upgrade-to-easy-faqs-pro/?utm_source=list_faqs_widget&utm_campaign=up
					grade" target="_blank"><?php echo $easy_faqs->get_str('FAQ_UPGRADE_TEXT'); ?></a><?php endif; ?> </span>
				</label></p>
			</fieldset>				
		</div>
	...
	function update($new_instance, $old_instance){
		...
		$instance['categories_accordion_style'] = $new_instance['categories_accordion_style'];
	...
	function widget($args, $instance){
		...
		$categories_accordion_style = empty($instance['categories_accordion_style']) ? null : $instance['categories_accordion_style'];
		...
		$atts = array(
			...
			'categories_accordion_style' => $categories_accordion_style
		);
		echo $easy_faqs->outputFAQsByCategory($atts);
